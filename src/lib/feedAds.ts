...
//
// 記事画面の最下部バナーがAdMobに「ナビゲーションと誤認する配置」として
// ポリシー違反を指摘されたため、収益を補う枠として一覧にも広告を置いている。
// 一覧の広告はスクロールで流れるためタブバー位置に固定されず、
// 上下の記事カードとはFeedAdCard側の余白・区切り線・ラベルで分離する。
//
// 適用範囲はHomeの「新着」(LatestList)のみ。人気/ランキング/検索/MyPageには入れない。

// ファーストビューは純粋なコンテンツにする(先頭この件数までは広告を出さない)
const LEAD_ARTICLES = 3;
// 以降この件数ごとに広告を1枠挿入する。
// 当初6件ごとにしていたが、体感の広告密度が高かったため12件ごと(頻度を半分)に緩めた
