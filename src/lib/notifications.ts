...
export async function registerPushToken(digestEnabled: boolean): Promise<boolean> {
...
export async function requestPermissionAndRegister(digestEnabled: boolean): Promise<boolean> {
  ...
  return registerPushToken(digestEnabled);
}

// 起動時同期: 許諾済み端末のみ、トークンローテーションに追随してサーバへ再登録する。
// 未許諾端末では何もしない(OSプロンプトはプレ許諾フロー以外から出さない)
export async function syncPushTokenIfGranted(digestEnabled: boolean): Promise<void> {
  const current = await Notifications.getPermissionsAsync();
  if (!current.granted) {
    return;
  }
  await ensureAndroidChannel();
  await registerPushToken(digestEnabled);
}
