...
### ストアコンソール側の作業(人手・必須)
- [x] **AdMob GDPRメッセージ(2026-07-26 公開済み)**: メッセージ「おすすもう GDPR 同意メッセージ」が
      まとめくん iOS/Android を含む4アプリを対象に公開済み。PP URLは各アプリに設定済み。
      アカウント設定(広告パートナー198社自動選択・正当な利益コントロール)はGoogle既定値で確認済み
