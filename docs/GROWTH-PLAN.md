...
- **ただし訴求としては使わない。** 「広告カット」を掲げると、AdMobで収益化していることと矛盾し、
  ...
  いずれも競合に存在せず、かつ自社が生成した情報なので、掲げても矛盾しない
...
- 競合レビュー欄の不満(広告過多・更新停止・サイト数)を説明文で切り返す。
  **ただし「広告過多」への切り返しは避ける**(自分も広告を出しているため)。
  更新停止とサイト数(69)、読み上げの有無で差をつける
...
| まとめ市場自体の縮小 | 差別化点(**話題検知・読み上げ・69サイト横断**)をASOで前面に。市場内シェア奪取で10倍は狙える規模(競合レビュー数千件)。整形の読み口は訴求に使わない(上記⚠️) |
