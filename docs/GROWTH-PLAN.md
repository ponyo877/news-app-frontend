...
- 端末上スクレイピングによる整形(サイドバー・目次・レコメンド枠を省いた読み口)は、競合(Marimba2log等)が月額課金で売っている価値に相当する
- **ただし訴求としては使わない。** 「広告カット」を掲げると、AdMobで収益化していることと矛盾し、
  他パブリッシャーの広告を除去していると自ら宣言する形になる。2026-08-14のAdMob違反はこれが引き金だった
  (詳細は REMAINING_TASKS.md)。**打ち出す差別化点は「話題検知・読み上げ・69サイト横断ランキング」**とする —
  いずれも自社が生成した情報なので、掲げても矛盾しない
- ⚠️ **ただし「競合に存在しない」のは話題検知だけ。** 2026-08-20に Play を実地調査した結果:
  - 読み上げは **2ch/5chまとめ 読み上げ+**(RANRANソリューションズ)が自動スクロール付きで実装済み。
    規模は500+DLと小さいが「唯一」「初」とは書けない
  - **サイト数は差別化にならない。** 評価1位(4.8/10万DL)の lifeofsnufkin は掲載100以上で、69は劣位
  - 詳細と、競合レビューの不満への切り返し文言は **docs/COMPETITORS.md**
...
> ⚠️ **「広告カット」「広告なし」を訴求に使わないこと。** 2026-08-13にアプリ名を
> 「まとめくん 2ch・5chまとめ 広告なしで読み上げ」に、説明文を「広告と目次をカット」に変えた翌日、
> AdMobから「サイトの動作: ナビゲーション」違反を受けた。Playの公開ページはアプリ名「広告なし」の直下に
> Googleの「広告が表示されます」バッジが並ぶ矛盾した状態だった。2026-08-19に撤回済み(REMAINING_TASKS.md)。

- スクリーンショット: RN版UIで撮り直し(現状旧Flutter UIの可能性大)。1枚目は
  **「読み上げでながら聴き」または「複数サイトが同じスレをまとめたら検知」**の価値訴求
- タイトル/サブタイトル: 検索キーワード(2chまとめ/5chまとめ/まとめサイト)を含める。
  現行は「まとめくん 2ch・5chまとめ 読み上げビューア」(25/30字)
- 説明文冒頭3行に差別化点(**69サイト横断・読み上げ・話題検知**)。整形の話をするなら
  「本文だけを読みやすく」まで。除去の対象を列挙しない
- `app-ads.txt` 設置 — ✅ **2026-08-19 完了**(`news-app-infra` の nginx で app へプロキシ、200を確認)
- 競合レビュー欄の不満を説明文で切り返す — ✅ **2026-08-20 調査完了(docs/COMPETITORS.md)**。
  実際に最も多かった不満は広告過多でもサイト数でもなく **Cookie同意ポップアップが閉じられない**ことだった
  (Marimba・lifeofsnufkin の両方で独立に発生。「使い物にならないんでアンインストールします」)。
  まとめくんは全サイトで `script`/`noscript` を除去してから描画するため構造的にこれを踏まない。
  次点は**画像が表示されない**(43人が役立ったと評価)で、こちらも画像を消していないので切り返せる。
  **どちらも「広告」の語を使わずに同じ利益を伝えられる**のが重要
...
| まとめ市場自体の縮小 | 差別化点(**話題検知**を筆頭に、読み上げ・横断ランキング)をASOで前面に。市場内シェア奪取で10倍は狙える規模(1位アプリは4.8/6400レビュー/10万DL)。整形の読み口は訴求に使わない(上記⚠️)。ただし読み上げは競合1本が実装済み、サイト数は劣位(docs/COMPETITORS.md) |
| **ASO文言がAdMobポリシーに抵触** | 「広告なし」「広告カット」を掲げない。ストア文面を変える前に、AdMobで収益化している事実と矛盾しないか確認する。2026-08-14に実際に発生 |
...
- [x] 2026-08-27: ⚡関連記事の「ほぼ同じ記事」対策(Workerのみ・アプリ変更なし)。同じネタの除外
      (今の記事・直近閲覧・表示済みと cos ≥ 0.75)+ MMR(λ=0.7)+ 同一サイト上限3 + 埋め込み用タイトル正規化
      (【タグ】・ｗ・記号連打を除去)。候補 topK 20→30(v2 の上限は values 付きで50。課金は topK 非依存。CPU 10ms 配慮で30)。
      本番較正: 改題は 0.77〜0.86、同方向の別記事は 0.73〜0.74。`debug:true` で判定根拠を返す
- [x] 2026-08-27: 同じ rerank を for You にも適用(anchor なし・読んだネタの別サイト版を除外・MMR・サイト上限3・候補30/最大20件)
- [x] 2026-08-27: Vectorize 掃除の取りこぼし修正(毎時15分tick判定→KV時刻ベース30分間隔。期限切れ約2,470本が滞留していた。
      実流量は約1,900件/日で48h保持なら≈3,800本)
